...
import { FaBars, FaTimes, FaUserTie, FaBuilding, FaChartLine, FaImages, FaVideo, FaBriefcase } from "react-icons/fa"
import { LanguageContext } from "../context/LanguageContext"
export default function Navbar() {
 const { lang, toggleLang } = useContext(LanguageContext)
 const [open, setOpen] = useState(false)
 const ar = lang === "ar"
 const links = [["/#about",ar?"عن الدكتور":"About",FaUserTie],["/#company",ar?"الشركة":"Company",FaBuilding],["/#skills",ar?"الخبرات":"Expertise",FaChartLine],["/#gallery",ar?"المعرض":"Gallery",FaImages],["/videos",ar?"الفيديوهات":"Videos",FaVideo],["/#youth",ar?"الفرص المهنية":"Careers",FaBriefcase]]
 ...
 <div className="ko-desktop-links">{links.slice(0,3).map(([to,label,Icon])=><Link key={to} to={to}><Icon aria-hidden="true"/>{label}</Link>)}</div>
 ...
 {open&&<div className="ko-navigation" id="ko-navigation">{links.map(([to,label,Icon])=><Link key={to} to={to} onClick={()=>setOpen(false)}><Icon aria-hidden="true"/><span>{label}</span></Link>)}</div>}
 </nav></header>
}
